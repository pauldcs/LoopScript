...
while getopts "p:s:i:o:m:vr:h" opt; do
    case $opt in
...
check_prerequisites() {
    if [ ! -d "$input_directory" ]; 
        then
            exit_with_error "\$input_directory (-i): Error"
    elif [ ! -r "$input_directory" ];
        then
            exit_with_error "\$input_directory (-i): Not readable"
    fi
    if [ -z "$(ls -A $input_directory/*.$input_file_suffix)" ];
        then
...
# MODE 0
# Executes the program with the content
# of the infile as arguments
__exec_file_content_as_args() {
    local input_file="$1"
...
# MODE 1
# Executes the program with the
# path to the infile as arguments
__exec_path_to_file_as_arg() {
    local input_file="$1"
    local actual_output_file="$2"
    local valgrind_log_file="$3"
    ...
    if [ "$run_under_valgrind" = true ];
        ...
        python3 "$program_name" "$input_file" &> "$actual_output_file"
    fi
    exit_code=$?
    return $exit_code
}
...
# MODE 2
# Supposed to be used with bash as $program
# this will execute the content of $infile as commands
__exec_file() {
    local input_file="$1"
    local actual_output_file="$2"
...
for file in "$input_directory"/*."$input_file_suffix"; 
    do
        ...
        actual_output_file="$output_directory/$test_name.out"
        valgrind_log_file="$output_directory/$test_name.valg"
        expected_output_file="$input_directory/$test_name.out"
...
if [ "$failed" -eq 0 ] && [ "$memory_errors" -eq 0 ];
    then
        printf "$OK_COLOR"
    ...
    printf "$ERROR_COLOR"
    printf "Errors were detected, check above for details.\n"
    printf "$NO_COLOR"
fi
